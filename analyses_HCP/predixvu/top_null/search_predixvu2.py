...
with the *top N* genes per phenotype *permutation*. 
...
gene_path = '/data1/rubinov_lab/brain_genomics/analyses_HCP/predixvu/top_null/symbol_ensembl_region' 
brain_path = '/data1/rubinov_lab/brain_genomics/analyses_HCP/predixvu/brain_phecode_defns.csv'

out_path = '/data1/rubinov_lab/brain_genomics/analyses_HCP/predixvu/top_null/phecodes_genes' 
except_path = '/data1/rubinov_lab/brain_genomics/analyses_HCP/predixvu/top_null/need_run' 
...
phens_perms = [(phen, i) for phen in phens for i in range(100)]
for (phen, perm) in phens_perms: 
    phe_dict = {} ## k: phecode, v: [gene symbols]

    ## gather genes to query 
    plist = '{}/list_{}_{}.txt'.format(except_path, phen, perm) 
    ...
    with open('{}/{}_{}.txt'.format(out_path, phen, perm), 'r') as f: 
    ...
    for pcode, genes in phe_dict.items(): 
        ...
        if key in old_keys: 
            ...
            results[key] = genes 
            print('{}-{}: added new med phen'.format(phen, perm))
