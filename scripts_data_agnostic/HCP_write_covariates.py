...
cov_file = '/data1/rubinov_lab/brain_genomics/analyses_HCP/DATA_OUTPUT/covariates.txt'

## covariates dictionary 
covs = ['race', 'age', 'gender', 'PC1', 'PC2']
cdict = {c:[] for c in covs}

## get subject IDs for HCP 890 cohort 
with h5py.File(sub_file, 'r') as f: 
    subjects = np.array(f['subjects']).astype(str) 
...
## parse PCA file 
pcs = np.loadtxt('{}/hcp_cohort.pca'.format(pca_path), skiprows=4)
pc1 = pcs[:,0]
pc2 = pcs[:,1]
