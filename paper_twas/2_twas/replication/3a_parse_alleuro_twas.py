...
disc_path = f'{main_path}_UKB/repl_HCPallEuro' ## /{itr}_{reg}.txt 
repl_path = f'{main_path}_UKB/repl_HCPallEuro' ## /repl_{itr}_{reg}.txt

ntwn_path = f'{main_path}_HCP/allEuro/twas_JTI/vol_mean' ## /{reg}.txt 

outs_path = f'{main_path}_UKB/HCPallEuro_summary.csv'
...
stats = pd.DataFrame([], columns=['reg', 'itr', 'n_disc', 'ukb_repl', 'hcp_alleuro'])
...
for reg in regs: 
    ...
    for i in range(n_itrs): 
        ...
        data = {'reg': reg, 'itr': i, 'n_disc': ds, 'ukb_repl': ukb, 'hcp_alleuro': hcp}
