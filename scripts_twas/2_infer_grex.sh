...
## Script for running any imputation models (only the 10 brain 
...
genotyp="${top_dir}/scripts_twas/inputs_${GROUP}/dosage_${MODEL}/c*.dosage.txt"

## HCP samples 
samples="${top_dir}/scripts_twas/inputs_HCP/genotype_samples.txt"

## UKB samples
samples="${top_dir}/scripts_twas/inputs_UKB/cohort_filtered.txt"
...
## output GREx path
outgrex="${top_dir}/scripts_twas/outputs_${GROUP}/grex_${MODEL}"
...
model_names=(Hippocampus Amygdala Caudate_basal_ganglia \
            ...
            Cerebellar_Hemisphere) # Frontal_Cortex_BA9 Anterior_cingulate_cortex_BA24)
...
do 
    ...
    --prediction_summary_output ${outgrex}/${short_names[$idx]}.log \
    --only_entries ${query} &
